...
export default function OvertimePage() {
  ...
  // Data hooks
  const { summary, isLoading: summaryLoading, reload: reloadSummary } = useOvertimeSummary(year, month)
  const { records: myRecords, isLoading: myLoading, reload: reloadMy } = useMyOvertime()
  const { records: teamRecords, isLoading: teamLoading, reload: reloadTeam } = useTeamOvertime(isLeaderOrManager)

  // Action hooks
  const { isSubmitting: isCreating, createOvertime } = useCreateOvertime(async () => {
    setRegisterOpen(false)
    await Promise.all([reloadSummary(), reloadMy()])
  })

  const { isSubmitting: isReviewing, reviewOvertime } = useReviewOvertime(async () => {
    await reloadTeam()
  })
    ...
    <div className="container mx-auto py-8 px-4 max-w-7xl space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex flex-col gap-1">
          <h1 className="text-3xl font-bold tracking-tight">Control de Horas</h1>
